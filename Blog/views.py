...
def blog_page(request):
    if request.user.is_authenticated:
        if request.user.is_superuser:
            ...
            get = Blog.objects.filter(author=request.user)
...
def contact_page(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            CT = contactTable(name=name, email=email,
                              phoneno=phoneno, message=message)
            CT.save()
            messages.add_message(
                request, messages.SUCCESS, "Thank You For Contacting Us, we will get back soon!")
    else:
...
def signup_page(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.save()
            messages.add_message(
                request, messages.SUCCESS, "Congratulations Your Account Has Been CreatedSuccessfully!")
    else:
